...
$sqlGetRecentRecords = "SELECT * FROM book_issue ORDER BY id DESC LIMIT 0,10";
$stmt = $conn->prepare($sqlGetRecentRecords);
$stmt->execute();
$result = $stmt->fetchAll(PDO::FETCH_ASSOC);

if(isset($_GET['id']) && $_GET['id'] > 0){
    $sqlGetRecentRecord = "SELECT * FROM book_issue WHERE id = '".$_GET['id']."'";
...
if(isset($_GET['action']) && $_GET['action'] == 'delete'){
    $sqlDelete = "DELETE FROM book_issue WHERE id = '".$_GET['id']."'";
    $stmt = $conn->prepare($sqlDelete);
    if($stmt->execute()){
        header('Location:'.$url.'books-add.php');
    }
}
...
if(isset($_POST['title']) && $_POST['title'] != ""){
    ...
    try {
        ...
        if(isset($_POST['submit'])){
            ...
            header('Location:'.$url.'books-add.html');
            exit;
        }
            ...
            <div class="row">
                <div class="col-lg-12 align-items-center section-padding">
                    <div class="hero-body" data-aos="fade-up">
                        <h3 class="text-uppercase sub-header">Add Book Issue
                    ...
                    <div class="col-12">
                        <form name="current-issue-add" method="post" action="" enctype="multipart/form-data">
                            <table class="table table-striped">
                                ...
                                <tr>
                                    <td>Publish Date</td>
                                    <td><input type="date" required class="form-control" name="publish_date" value="<?php echo $publish_date;?>" /></td>
                                </tr>
                                <tr>
                                    <td>Order</td>
                ...
                if($result && $result != false){
                    ...
                    foreach($result as $data){
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo $url;?>/books-add.html?id=<?php echo $data['id'];?>">Edit</a>
    ...
    <script type="text/javascript">
        function fnDeleteConfirm(id){
            if(!confirm("Are you sure?")){
                ...
                window.location.href = '<?php echo $url;?>/books-add.php?action=delete&id='+id;
            }
        }
        function openPopup() {
